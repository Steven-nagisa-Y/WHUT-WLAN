...
def getAcId() -> int:
    response = session.get(
        'http://edge.microsoft.com/captiveportal/generate_204?cmd=redirect&arubalp=12345')
    match_list = re.findall(
        r"<meta http-equiv='refresh' content='1; url=(.*?)'>", response.text, re.S)
    if len(match_list) == 0:
        url = response.url
    else:
        url = match_list[0]
    ...
    url = url.replace('index_' + numStr + '.html',
                      'srun_portal_pc.php?ac_id=' + numStr)
    response = session.get(url)
    match_list = re.findall(
        r'<input type="hidden" name="ac_id" value="(.*?)">', response.text, re.S)
    if len(match_list) == 0:
        return -1
...
def logout(username, password):
    postData = {"action": "logout", "username": username,
                "password": password, "ajax": 1}
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        try:
            ...
            print("Pass your cardid and password to me!")
    ...
    while True:
        try:
            ...
            logging.exception("Let me sleep for 10 seconds")
            logging.info("ZZzzzz...")
            time.sleep(10)
            logging.info("Was a nice sleep, now let me continue...")
            continue
    i = input("You can close me now... $ ")
    import os
    os.system(i)
